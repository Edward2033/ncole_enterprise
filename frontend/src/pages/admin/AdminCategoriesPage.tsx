...
import { FolderOpen, Plus, Pencil, Trash2, Eye, EyeOff, RefreshCw } from 'lucide-react';
...
const EMPTY_FORM: CatForm = { name: '', slug: '', description: '', sortOrder: '0', parentId: '', isVisible: true, imageUrl: '' };
...
const AdminCategoriesPage: React.FC = () => {
  ...
  const handleNameChange = (name: string) => {
    setForm(f => ({ ...f, name, slug: editCat ? f.slug : name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '') }));
  };

  const openEdit = (cat: AdminCategory) => {
    setForm({ name: cat.name, slug: cat.slug, description: cat.description ?? '', sortOrder: String(cat.sortOrder), parentId: cat.parentId ?? '', isVisible: cat.isVisible, imageUrl: cat.imageUrl ?? '' });
    setEditCat(cat);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const body = { name: form.name, slug: form.slug, description: form.description || undefined, sortOrder: Number(form.sortOrder), parentId: form.parentId || undefined, isVisible: form.isVisible, imageUrl: form.imageUrl || undefined };
      if (editCat) {
  ...
  const handleDelete = async () => {
    if (!delCat) return;
    setSaving(true);
    try { await adminCategoriesApi.delete(delCat.id); setDelCat(null); await load(); }
    catch (e) { setError((e as Error).message); }
    finally { setSaving(false); }
  };

  const handleToggleVisibility = async (cat: AdminCategory) => {
    try { await adminCategoriesApi.update(cat.id, { isVisible: !cat.isVisible }); await load(); }
    catch (e) { setError((e as Error).message); }
  };
  ...
  const columns: Column<AdminCategory>[] = [
    {
      ...
      render: c => (
        <div className="flex items-center gap-3">
          {c.imageUrl
            ? <img src={c.imageUrl} alt={c.name} className="h-9 w-9 rounded-lg object-cover border border-slate-200 dark:border-slate-700 flex-shrink-0" />
            : <div className="h-9 w-9 rounded-lg bg-slate-100 dark:bg-slate-800 flex items-center justify-center flex-shrink-0"><FolderOpen className="h-4 w-4 text-slate-400" /></div>
          }
          <div>
    ...
    { key: 'added', header: 'Created',     render: c => <span className="text-xs text-slate-500">{fmtDate(c.createdAt)}</span> },
    {
      ...
      render: c => (
        <div className="flex items-center gap-1">
          <button onClick={() => handleToggleVisibility(c)} title={c.isVisible ? 'Hide' : 'Show'} className="rounded-lg p-1.5 text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 transition">
            {c.isVisible ? <EyeOff className="h-4 w-4"/> : <Eye className="h-4 w-4"/>}
          </button>
          <button onClick={() => openEdit(c)} className="rounded-lg p-1.5 text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 transition">
            <Pencil className="h-4 w-4"/>
          </button>
          <button onClick={() => setDelCat(c)} className="rounded-lg p-1.5 text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition">
  ...
  const CategoryForm = () => (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="space-y-1">
          ...
          <input required value={form.slug} onChange={e => setForm(f => ({...f, slug: e.target.value}))}
            className="w-full rounded-xl border border-slate-200 dark:border-slate-700 dark:bg-slate-800 dark:text-white px-3 py-2 text-sm outline-none focus:border-orange-400 font-mono"/>
        </div>
        <div className="col-span-2 space-y-1">
          <label className="block text-xs font-medium text-slate-700 dark:text-slate-300">Description</label>
          <textarea value={form.description} onChange={e => setForm(f => ({...f, description: e.target.value}))} rows={2}
            className="w-full rounded-xl border border-slate-200 dark:border-slate-700 dark:bg-slate-800 dark:text-white px-3 py-2 text-sm outline-none focus:border-orange-400 resize-none"/>
        </div>
        <div className="space-y-1">
          <label className="block text-xs font-medium text-slate-700 dark:text-slate-300">Sort Order</label>
        ...
        <div className="col-span-2 space-y-1">
          <label className="block text-xs font-medium text-slate-700 dark:text-slate-300">Image URL</label>
          <input value={form.imageUrl} onChange={e => setForm(f => ({...f, imageUrl: e.target.value}))}
            placeholder="https://example.com/category.jpg"
            className="w-full rounded-xl border border-slate-200 dark:border-slate-700 dark:bg-slate-800 dark:text-white px-3 py-2 text-sm outline-none focus:border-orange-400"/>
          {form.imageUrl && (
            <img src={form.imageUrl} alt="preview" className="mt-2 h-16 w-16 rounded-xl object-cover border border-slate-200 dark:border-slate-700"
              onError={e => { (e.target as HTMLImageElement).style.opacity = '0.2'; }} />
          )}
        </div>
        <div className="col-span-2 flex items-center gap-2">
          <input type="checkbox" id="visible" checked={form.isVisible} onChange={e => setForm(f => ({...f, isVisible: e.target.checked}))} className="rounded"/>
      ...
      <div className="flex gap-2">
        ...
        <button type="button" onClick={() => { setShowCreate(false); setEditCat(null); setForm(EMPTY_FORM); }}
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="flex items-center gap-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-amber-100"><FolderOpen className="h-5 w-5 text-amber-600"/></div>
          <div>
      ...
      {/* Create */}
      <AdminModal open={showCreate} onClose={() => { setShowCreate(false); setForm(EMPTY_FORM); }} title="New Category">
        <CategoryForm/>
      </AdminModal>

      {/* Edit */}
      <AdminModal open={!!editCat} onClose={() => { setEditCat(null); setForm(EMPTY_FORM); }} title="Edit Category">
        <CategoryForm/>
      </AdminModal>
