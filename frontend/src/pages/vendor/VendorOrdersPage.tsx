...
import { Search, ChevronDown, X, Package, MapPin, User, CreditCard, FileText } from 'lucide-react';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        ...
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 px-6 py-4">
          <div>
            <h2 className="text-lg font-bold dark:text-white">{order.orderNumber}</h2>
          ...
          <div className="flex items-center gap-2">
            ...
            <PBadge className={PAYMENT_STATUS_COLOR[order.paymentStatus] ?? 'bg-slate-100 text-slate-600'}>
              {order.paymentStatus}
            </PBadge>
        ...
        <div className="p-6 space-y-5">
          ...
          {order.customer && (
            <section>
              ...
              <div className="rounded-xl bg-slate-50 dark:bg-slate-800 px-4 py-3 grid grid-cols-2 gap-2 text-sm">
                <div>
                  <p className="text-xs text-slate-400">Name</p>
                  <p className="font-medium dark:text-white">{order.customer.user.name}</p>
                </div>
                <div>
                  <p className="text-xs text-slate-400">Email</p>
                  <p className="font-medium dark:text-white">{order.customer.user.email}</p>
                </div>
          ...
          {order.address && (
            <section>
              ...
              <div className="rounded-xl bg-slate-50 dark:bg-slate-800 px-4 py-3 text-sm">
                <p className="font-medium dark:text-white">{order.address.fullName} &middot; {order.address.phone}</p>
                <p className="text-slate-500 mt-0.5">{order.address.street}, {order.address.district}, {order.address.city}</p>
                <p className="text-slate-400 text-xs mt-0.5">{order.address.province}, {order.address.country}</p>
              </div>
            </section>
          )}
                ...
                return (
                  <div key={item.id} className="flex items-start gap-3 rounded-xl bg-slate-50 dark:bg-slate-800 p-3">
                    ...
                    <div className="flex-shrink-0 text-right">
                      ...
                      {item.product && (
                        <p className="text-xs text-slate-400 mt-0.5">Stock: {item.product.stockQty}</p>
                      )}
                    </div>
                  </div>
                );
          ...
          <section>
            ...
            <div className="rounded-xl bg-slate-50 dark:bg-slate-800 px-4 py-3 space-y-1.5 text-sm">
              ...
              <div className="flex justify-between font-bold border-t border-slate-200 dark:border-slate-700 pt-1.5 dark:text-white">
                <span>Total</span>
...
const VendorOrdersPage: React.FC = () => {
  ...
  const [detail, setDetail] = useState<NcoleVendorOrder | null>(null);
  ...
  const handleStatusUpdate = async (id: string, status: OrderStatus) => {
    ...
    setUpdating(null);
    // Refresh and keep detail modal open with updated data
    vendorOrdersService.list(1, 20).then(res => {
      setOrders(res.data);
      if (detail) setDetail(res.data.find(o => o.id === id) ?? null);
    }).catch(() => null);
  };
    ...
    <div className="space-y-6">
      ...
      {visible.length === 0 ? (
        ...
        <div className="space-y-3">
          {visible.map(order => {
            const action = VENDOR_ACTIONS[order.status as OrderStatus];
            // Show up to 3 product thumbnails in the card
            const thumbs = order.items.slice(0, 3);
            return (
              <PCard key={order.id} className="cursor-pointer hover:shadow-md transition-shadow" onClick={() => setDetail(order)}>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex -space-x-2">
                      {thumbs.map(item => {
                        ...
                        return (
                          <div key={item.id} className="h-10 w-10 flex-shrink-0 overflow-hidden rounded-xl border-2 border-white dark:border-slate-900 bg-slate-100">
                            {img ? (
                      ...
                      {order.items.length > 3 && (
                        <div className="flex h-10 w-10 items-center justify-center rounded-xl border-2 border-white dark:border-slate-900 bg-slate-200 dark:bg-slate-700 text-xs font-bold text-slate-600 dark:text-slate-300">
                          +{order.items.length - 3}
                        </div>
                      )}
                    </div>
